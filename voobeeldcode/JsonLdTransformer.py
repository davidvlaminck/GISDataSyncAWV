...
class JsonLdTransformer:
    context_dir = dict(
        ...
        bs='https://bs.data.wegenenverkeer.be/ns/implementatieelement#',
        ...
        purl='http://purl.org/dc/terms/', schema='https://schema.org/')

    def __init__(self, resource_type: ResourceEnum, otl_db_path: Path = Path(CURRENT_DIR / 'OTL.db')):
    ...
    def transform_ld_dict(self, original_ld_dict) -> dict:
        new_dict = {}
        for k, v in original_ld_dict.items():
            if isinstance(v, dict):
                v = self.transform_ld_dict(v)
            elif isinstance(v, list):
                new_list = []
                for item in v:
                    if isinstance(item, dict):
                        new_list.append(self.transform_ld_dict(item))
                    else:
            ...
            if ':' in k:
                ...
                new_dict[
                    'https://wegenenverkeer.data.vlaanderen.be/ns/implementatieelement#' + k] = \
                    self.transform_if_http_value(v)
    ...
    def enhance_asset_graph(self, asset_graph):
        ...
        for asset in asset_graph['@graph']:
            fixed_dict = self.transform_ld_dict(asset)
            if self.type == ResourceEnum.assets:
                fixed_dict = JsonLdTransformer.add_exact_geometry(fixed_dict)
    ...
    def add_exact_geometry(asset_dict):
            ...
            if len(asset_dict['geo:Geometrie.log']) > 0:
                ...
                if len(geom.items()) > 0:
                    ...
                    asset_dict['http://example.org/ApplicationSchema#hasExactGeometry'] = \
                        JsonLdTransformer.get_exact_geometry_from_wkt_str(wkt_str, node_id=asset_dict['@id'])
                return asset_dict
        ...
        if 'loc:Locatie.puntlocatie' in asset_dict and 'loc:3Dpunt.puntgeometrie' in \
                asset_dict['loc:Locatie.puntlocatie']:
            if 'loc:DtcCoord.lambert72' not in asset_dict['loc:Locatie.puntlocatie']['loc:3Dpunt.puntgeometrie']:
                return asset_dict
            coordinates = asset_dict['loc:Locatie.puntlocatie']['loc:3Dpunt.puntgeometrie']['loc:DtcCoord.lambert72']
            x = coordinates['loc:DtcCoordLambert72.xcoordinaat']
            y = coordinates['loc:DtcCoordLambert72.ycoordinaat']
            z = coordinates['loc:DtcCoordLambert72.zcoordinaat']
    ...
    @staticmethod
    def get_exact_geometry_from_wkt_str(wkt_str, node_id):
        geom_type = None
